...
def get_leads_fields(access_token, instance_url):
    """Fetch Leads fields from Salesforce, including their data types."""
    headers = {"Authorization": f"Bearer {access_token}"}
    describe_url = f"{instance_url}/services/data/v58.0/sobjects/Lead/describe"
    ...
    if response.status_code == 200:
        ...
        print("Error fetching Leads fields:", response.status_code, response.text)
        return None

def save_to_csv(fields_info, filename="leads_field_names_in_csv.csv"):
    """Save Salesforce Lead fields and their data types to a CSV file."""
    if fields_info:
...
if __name__ == "__main__":
    ...
    if access_token and instance_url:
        fields_info = get_leads_fields(access_token, instance_url)
