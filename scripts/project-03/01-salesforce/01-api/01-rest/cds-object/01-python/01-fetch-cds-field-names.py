...
def get_cds_fields(access_token, instance_url):
    """Fetch CDS fields from Salesforce, including their data types."""
    headers = {"Authorization": f"Bearer {access_token}"}
    describe_url = f"{instance_url}/services/data/v58.0/sobjects/Customer_Data_Survey__c/describe"
    ...
    if response.status_code == 200:
        ...
        print("Error fetching CDS fields:", response.status_code, response.text)
        return None

def save_to_csv(fields_info, filename="cds_fields_in_csv.csv"):
    """Save Salesforce CDS fields and their data types to a CSV file."""
    if fields_info:
...
if __name__ == "__main__":
    ...
    if access_token and instance_url:
        fields_info = get_cds_fields(access_token, instance_url)
