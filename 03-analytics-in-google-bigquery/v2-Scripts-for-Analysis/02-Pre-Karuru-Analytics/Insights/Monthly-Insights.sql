...
previous_month as (
                  select distinct date_trunc(posting_date, month) as month,
                  company,
                  count(distinct territory) as territories,
                  --count(distinct sales_partner),
                  ...
                  WHERE date_trunc(posting_date, month) = '2023-06-01'
                  group by 1,2
                  order by 1,2
                  ),
prior_month as (
                  ...
                  sum(grand_total) / count(distinct territory) as gmv_per_territory
                  from paid_and_delivered_dns
                  WHERE date_trunc(posting_date, month) = '2023-05-01'
                  group by 1,2
                  order by 1,2
                  )
